...
- 后续整改：最终删除 `schema.py`、领域规则抽取、人员专用身份索引和 pattern 兼容查询，统一为开放域 GraphDocument、严格 GraphPlan、关系目录白名单和参数化模板
- 验证：单测 72 passed；三类 ReAct 端到端均通过

### Graph RAG 渐进改造留下“假通用”兼容层
- 时间：2026-08-22
- 现象：抽取模型声称通用，但 catalog、IdentityIndex、入库约束和查询 fallback 仍绑定 Person / Service / Step；部分 GraphPlan 字段定义后未执行
- 根因：为了保旧测试采用双轨兼容，却没有逐层审计“抽取→身份→入库→规划→查询→生成”
- 处理：删除旧双轨；所有文档先抽取成功再单事务替换；实体类型和关系规范化；机器 ID 映射显示名；未知关系 fail-closed；五类 intent 使用真实模板
- 验证：重建后 3 个来源齐全、无重复实体、无缺失主键，并存在 REPORTS_TO / DEPENDS_ON / NEXT_STEP；三类 ReAct 查询均有正确路径或属性证据

...
| 项 | 当前值（截至 2026-08-27） |
|----|---------------------------|
| 场景 | 虚构「星云科技」内部制度 / FAQ / 通讯录 / PDF 手册 / 组织关系 / **架构图与看板截图** |
| 语料规模 | 文档侧 **14** 篇 → **72** 块；关系侧 **3** 篇 MD；多模态 **3** 张 PNG（VLM caption → **15** chunk） |
| 图谱规模 | 开放域 GraphDocument；样例含汇报/依赖/审批等 |
| 知识库 | **五库** policies / tabular / pdf / multimodal（向量）+ relations（Neo4j）；物理分库 |
| 存储 | 向量 `qdrant\|chroma`；关键词 `opensearch\|pkl`；图 **Neo4j**；CI 默认 Chroma + pkl |
| 入库 | `--ingest` 增量（含图像 caption 缓存）；`--ingest-graph` 增量（hash + pipeline_version） |
| 回归题集 | golden 含关系题 + **3** 道 visual 路由题 |
| 答案通过率 | **33/34**（直连 eval；关系题 skip；视觉题走 ReAct/路由专项） |
| 检索召回 | **31/31**（文档侧） |
| 路由选型 | **12/12**（含 `query_relations` ×3、`search_visual` ×3） |
| 图 vs 文档对照 | 关系题图检索 **3/3** |
| ReAct 端到端 | 关系题 + visual 手测通过；`run_react.py` 子集历史 **6/7** |
| 单测 | **77** passed（含 vision caption 缓存/失败、CRAG 控制流） |
| **默认演示** | **`--react` ReAct** |
| 辅助路径 | `--query` · `--agent` |
...
**已做完的**：入库（含 PDF、**图像 VLM caption**）、**物理分库 + Profile**、**可切换存储**、**增量入库**、混合检索、重排、带来源的回答、规则+模型两层拒答、多轮改写、**Agent / ReAct**、**Graph RAG**、**多模态 KB（`search_visual`）**、**CRAG Profile 钩子**、可观测、可重复评测。

**还没做的**（被问到如实说）：复合 PDF/PPT 版面拆解（文+表+图同文档）、回答阶段回灌原图、前端上传与异步入库队列、扩大 ReAct golden、Ragas、图谱时效字段、CLIP 双塔。
